...
class StockCreateTestCase(TestCase):
    def test(self):
        ...
        company.save()
        stock = Stock(date='2021-1-1', company=company,
                      vol=1, high=7, low=1, open=1, close=4)
        self.initialize(stock, company)
        stock.save()
        stock = Stock(date='2021-1-2', company=company,
                      vol=1, high=2, low=1, open=1, close=2)
...
class CalcAverages(TestCase):
    def calc_52_day_average(self, ticker, date):
        s = Stock(date='2020-1-4', symbol='AAPL', name="Apple",
                  vol=1, high=1, low=1, avg=2, open=1, close=1)
        s.save()
        s = Stock(date='2020-1-1', symbol='AAPL', name="Apple",
                  vol=1, high=1, low=1, avg=2, open=1, close=4)
        s.save()
        s = Stock(date='2020-1-2', symbol='AAPL', name="Apple",
                  vol=1, high=1, low=1, open=1, avg=0, close=2)
        s.save()
...
class StockChangeTest(TestCase):
    def calc_percent_changes(self):
        s = Stock(date='2020-3-21', symbol='AAPL', name="Apple",
                  vol=1, high=1, low=1, avg=2, open=1, close=1)
        s.save()
        s = Stock(date='2020-3-20', symbol='AAPL', name="Apple",
                  vol=1, high=1, low=1, avg=2, open=1, close=1)
        s.save()
        ...
        for stock in stocks:
            if Stock.objects.filter(symbol=stock.symbol, date=yesterday).exists():
                prev_stock = Stock.objects.get(
                    symbol=stock.symbol, date=yesterday)
                ...
                stock_change = StockChange(
                    stock=stock, date=stock.date, vol=vol, high=high, low=low, avg=avg, open=open, close=close)
...
class StockUpdateAndChange(TestCase):
    def execute(self):
        s = Stock(date='2020-3-22', symbol='AAPL', name="Apple",
                  vol=1, high=1, low=1, avg=2, open=1, close=1)
        s.save()
